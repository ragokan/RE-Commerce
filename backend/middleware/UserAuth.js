...
const noPermissionError = (next) =>
  next(new ErrorObject("You don't have permission to do this action!", 401, 102));
...
const LoginRequired = Async(async (req, res, next) => {
  ...
  if (checkToken)
    return next(new ErrorObject("User already logged out, to do this action log in!", 401, 103));
